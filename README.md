...
首版针对 macOS 与已验证的 Codex CLI 0.144.x 内部状态；内部格式并非公开 API，Codex 更新后工具可能为安全起见拒绝写入。所有写命令默认只演练，必须关闭 Codex 并显式加 `--yes`。备份位于 `$CODEX_HOME/rehome-backups`，可能包含敏感对话，权限设为 0700/0600。失败会自动恢复；也可在关闭 Codex 后运行 `codex-rehome rollback <migration-id> --yes`。
