...
Controls include component-boundary matching, semantic allowlists, schema gates, explicit `--yes`, process refusal, exclusive locking, SQLite transactions, atomic same-directory JSON replacement, SHA-256 verification, 0700/0600 backup permissions, post-write reload, automatic restoration, and conflict-safe rollback. Backups remain sensitive and should be deleted only after the user is satisfied with recovery.
